...
class ShapeMemorySpace {
    ...
    async uploadPhoto(file) {
        ...
        const filename = `memories/${timestamp}_${file.name}`;
    ...
    async deletePhoto(photoId) {
        try {
            ...
            // Find and delete from Firebase Storage
    ...
    async clearAllPhotos() {
        ...
        try {
            ...
            const storageDeletePromises = this.photos.map(photo => {
                if (photo.url) {
                    const storageRef = storage.refFromURL(photo.url);
                    return storageRef.delete().catch(console.error);
                }
                return Promise.resolve();
            });
            
            await Promise.all([...deletePromises, ...storageDeletePromises]);
